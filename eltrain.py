...
def run(args):
    ...
    train_dataset = MRDataset('../MRNet-v1.0/', args.task,args.plane, transform=augmentor, train=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=1, shuffle=True, num_workers=0, drop_last=False)

    validation_dataset = MRDataset( '../MRNet-v1.0/', args.task, args.plane, train=False)
    ...
    optimizer = optim.Adam(Elnet.parameters(), lr=args.lr)
    ...
    for epoch in range(num_epochs):
        ...
        if val_auc > best_val_auc:
            ...
            if bool(args.save_model):
                ...
                for f in os.listdir('../MRNet-v1.0/models/'):
                    if (args.task in f) and (args.plane in f) and (args.prefix_name in f):
                        os.remove(f'../MRNet-v1.0/models/{f}')
                torch.save(Elnet, f'../MRNet-v1.0/models/{file_name}')
